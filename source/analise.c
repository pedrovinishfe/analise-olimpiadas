...
void q1_total_medalhas_por_estacao(Atleta* atletas, int n_atletas, Resultado* resultados, int n_resultados, NocRegiao* regioes, int n_regioes) {
    char paises[10][10];
    int contagem[10][2]; // [0]=Verao, [1]=Inverno
...
// Q2: Removido 'char* codigo_pais' dos argumentos
...
// Q3: Removido 'char* codigo_pais, int ano' dos argumentos
...
// Q4: Removido 'int ano' dos argumentos
void q4_media_idade_medalhistas(Atleta* atletas, int n_atletas, Resultado* resultados, int n_resultados) {
    ...
    else printf("   [AVISO] Nenhum dado encontrado para %d.\n", ano);
    free(vis);
}
...
void q7_atletas_verao_e_inverno(Atleta* a, int na, Resultado* r, int nr) {
    ...
    printf("Processando... (Pode demorar um pouco)\n");

    char* ver=(char*)calloc(MAXIMO_ATLETAS,1);
    char* inv=(char*)calloc(MAXIMO_ATLETAS,1);

    // 1. Identificar quem participou em ambos
    for(int k=0; k<nr; k++) {
    ...
    for(int i=0; i<na; i++) {
        int id = a[i].id;
        if(id>=0 && id<MAXIMO_ATLETAS && ver[id] && inv[id]) {
            printf("[%d] %s (%s): ", count+1, a[i].nome, a[i].noc);

            int primeiro = 1;
            int anos_ja_listados[30]; int n_ja=0;

            for(int k=0; k<nr; k++) {
                if(r[k].id_atleta == id) {
                    int ja_foi = 0;
                    for(int z=0; z<n_ja; z++) if(anos_ja_listados[z] == r[k].ano_olimpiada) ja_foi=1;

                    if(!ja_foi) {
                        if(!primeiro) printf(", ");
                        printf("%d %s", r[k].ano_olimpiada, r[k].estacao);
                        primeiro = 0;
                        anos_ja_listados[n_ja++] = r[k].ano_olimpiada;
                    }
                }
            }
            printf("\n");
...
void q11_primeira_medalha_pais(Atleta* a, int na, Resultado* r, int nr) {
    ...
    if(min!=9999) printf("   Ano: %d (%s)\n", min, tipo); else printf("   Nenhuma.\n");
}

// Q12: 10 países escolhidos
void q12_ranking_paises_total_medalhas(Atleta* a, int na, Resultado* r, int nr, NocRegiao* re, int nre) {
    ...
    printf("\n=== Q12: RANKING 10 PAISES ESCOLHIDOS (TOTAL MEDALHAS) ===\n");
    ...
    gerar_grafico_barra("q12_ranking.svg", l, v, 10, "Ranking Paises", "Total");
}
...
void q13_top_atletas_maiores_medalhistas(Atleta* a, int na, Resultado* r, int nr, NocRegiao* re, int nre) {
    ...
    for(int i=0; i<nr; i++) if(strlen(r[i].medalha)>0 && strcmp(r[i].medalha,"NA")!=0) {
        if(r[i].id_atleta>=0 && r[i].id_atleta<MAXIMO_ATLETAS) c[r[i].id_atleta]++;
    }
    double v[10]; char* l[10];
    for(int k=0; k<10; k++) {
...
void q14_ranking_paises_participacao(Atleta* a, int na, Resultado* r, int nr, NocRegiao* re, int nre) {
    ...
    double v[10]; char* l[10];
    for(int i=0; i<10; i++) { printf("#%d %s: %d\n", i+1, paises[i], contagem[i]); v[i]=(double)contagem[i]; l[i]=paises[i]; }
    gerar_grafico_barra("q14_part.svg", l, v, 10, "Participacoes", "Qtd");
}

// Q15
void q15_ranking_ultimas_olimpiadas(Resultado* r, int nr) {
    printf("\n=== Q15: MEDALHAS (10 ULTIMAS) ===\n");
    ...
    gerar_grafico_barra("q15_ranking.svg", lb, v, lim, "Recentes", "Total");
}

// Q16: Removido 'int ano'
void q16_atletas_por_esporte_ano(Resultado* r, int nr) {
    char esportes[10][100]; int contagem[10]; int ano;
    char* vis = (char*)calloc(MAXIMO_ATLETAS, 1);

    printf("\n=== Q16: 10 ESPORTES ESCOLHIDOS ===\nDigite o Ano: "); scanf("%d", &ano);
    printf("Digite 10 esportes (Ingles, ex Judo):\n");
    for(int i=0; i<10; i++) { printf("Esp %d: ", i+1); scanf(" %99[^\n]", esportes[i]); contagem[i]=0; }

    for(int e=0; e<10; e++) {
        memset(vis, 0, MAXIMO_ATLETAS);
        for(int i=0; i<nr; i++) if(r[i].ano_olimpiada==ano && strstr(r[i].esporte, esportes[e]) && r[i].id_atleta>=0 && r[i].id_atleta<MAXIMO_ATLETAS && !vis[r[i].id_atleta]) {
            vis[r[i].id_atleta]=1; contagem[e]++;
        }
    }
    for(int i=0; i<9; i++) for(int j=i+1; j<10; j++) if(contagem[j]>contagem[i]) {
        int t=contagem[i]; contagem[i]=contagem[j]; contagem[j]=t;
        char ts[100]; strcpy(ts, esportes[i]); strcpy(esportes[i], esportes[j]); strcpy(esportes[j], ts);
    }
    double v[10]; char* lb[10];
    for(int i=0; i<10; i++) { printf("#%d %s: %d\n", i+1, esportes[i], contagem[i]); v[i]=(double)contagem[i]; lb[i]=esportes[i]; }
    gerar_grafico_barra("q16_esporte.svg", lb, v, 10, "Esportes", "Atletas");
    free(vis);
}

// Q17: Evolução Feminina
void q17_evolucao_participacao_feminina(Atleta* a, int na, Resultado* r, int nr) {
    ...
    printf("   [TABELA DE EVOLUCAO]\n");
    for(int ano=1896; ano<=2024; ano++) {
        int c=0, h=0; memset(vis,0,MAXIMO_ATLETAS);
        for(int i=0; i<nr; i++) if(r[i].ano_olimpiada==ano) {
            h=1;
            if(r[i].id_atleta>=0 && r[i].id_atleta<MAXIMO_ATLETAS && verifica_evento_feminino(r[i].evento) && !vis[r[i].id_atleta]) {
                vis[r[i].id_atleta]=1; c++;
            }
        }
        if(h) {
            printf("   Ano %d: %d mulheres\n", ano, c);
            if(p<200) { x[p]=ano; y[p]=c; p++; }
        }
    }
    printf("Gerado grafico historico.\n");
...
void q18_ranking_atletas_mais_velhos(Atleta* a, int na, Resultado* r, int nr) {
    printf("\n=== Q18: RANKING 10 ATLETAS MAIS VELHOS ===\n");
    int* max_idade_m = (int*)calloc(MAXIMO_ATLETAS, sizeof(int));
    int* max_idade_f = (int*)calloc(MAXIMO_ATLETAS, sizeof(int));
    ...
    for(int i=0; i<nr; i++) {
        ...
        if(id >= 0 && id < MAXIMO_ATLETAS) {
            ...
            if(idx != -1 && a[idx].ano_nascimento > 0) {
                int idade = r[i].ano_olimpiada - a[idx].ano_nascimento;
                if(verifica_evento_feminino(r[i].evento)) {
                    if(idade > max_idade_f[id]) max_idade_f[id] = idade;
                } else {
                    if(idade > max_idade_m[id]) max_idade_m[id] = idade;
                }
            }
        }
    }

    ParIdade* lm = (ParIdade*)malloc(sizeof(ParIdade) * MAXIMO_ATLETAS);
    ParIdade* lf = (ParIdade*)malloc(sizeof(ParIdade) * MAXIMO_ATLETAS);
    int cm=0, cf=0;

    for(int i=0; i<MAXIMO_ATLETAS; i++) {
        if(max_idade_m[i] > 0) { lm[cm].id_atleta = i; lm[cm].idade = max_idade_m[i]; cm++; }
        if(max_idade_f[i] > 0) { lf[cf].id_atleta = i; lf[cf].idade = max_idade_f[i]; cf++; }
    }
    qsort(lm, cm, sizeof(ParIdade), comparar_idade_decrescente);
    qsort(lf, cf, sizeof(ParIdade), comparar_idade_decrescente);

    printf("\n--- TOP 10 HOMENS MAIS VELHOS ---\n");
    for(int i=0; i<10 && i<cm; i++) {
        int idx = buscar_indice_atleta_por_id(a, na, lm[i].id_atleta);
        if(idx != -1) printf("%d. %s (%s): %d anos\n", i+1, a[idx].nome, a[idx].noc, lm[i].idade);
    }
    printf("\n--- TOP 10 MULHERES MAIS VELHAS ---\n");
    for(int i=0; i<10 && i<cf; i++) {
        int idx = buscar_indice_atleta_por_id(a, na, lf[i].id_atleta);
        if(idx != -1) printf("%d. %s (%s): %d anos\n", i+1, a[idx].nome, a[idx].noc, lf[i].idade);
    }
    free(max_idade_m); free(max_idade_f); free(lm); free(lf);
}

// Q19
void q19_ranking_medalhistas_mais_velhos(Atleta* a, int na, Resultado* r, int nr) {
    printf("\n=== Q19: RANKING 10 MEDALHISTAS MAIS VELHOS ===\n");
    int* max_idade_m = (int*)calloc(MAXIMO_ATLETAS, sizeof(int));
    int* max_idade_f = (int*)calloc(MAXIMO_ATLETAS, sizeof(int));
    ...
    printf("\n--- TOP 10 MEDALHISTAS HOMENS MAIS VELHOS ---\n");
    for(int i=0; i<10 && i<cm; i++) {
        int idx = buscar_indice_atleta_por_id(a, na, lm[i].id_atleta);
        if(idx != -1) printf("%d. %s (%s): %d anos\n", i+1, a[idx].nome, a[idx].noc, lm[i].idade);
    }
    printf("\n--- TOP 10 MEDALHISTAS MULHERES MAIS VELHAS ---\n");
    for(int i=0; i<10 && i<cf; i++) {
        int idx = buscar_indice_atleta_por_id(a, na, lf[i].id_atleta);
        if(idx != -1) printf("%d. %s (%s): %d anos\n", i+1, a[idx].nome, a[idx].noc, lf[i].idade);
    }
...
void q21_peso_medio_medalhistas_ouro(Atleta* a, int na, Resultado* r, int nr) {
    ...
    if(c>0) printf("   Media: %.2f kg\n", s/c); else printf("   Sem dados.\n"); free(vis);
}

// Q22
void q22_ranking_paises_medalhistas_unicos(Atleta* a, int na, Resultado* r, int nr, NocRegiao* re, int nre) {
    printf("\n=== Q22: RANKING PAISES (MEDALHISTAS UNICOS) ===\n");
    char* tem_med=(char*)calloc(MAXIMO_ATLETAS, 1);
    for(int i=0; i<nr; i++) if(strlen(r[i].medalha)>0 && strcmp(r[i].medalha,"NA")!=0) {
        if(r[i].id_atleta>=0 && r[i].id_atleta<MAXIMO_ATLETAS) tem_med[r[i].id_atleta]=1;
    }
    ParChaveValor l[500]; int n=0;
    for(int i=0; i<na; i++) {
        if(a[i].id>=0 && a[i].id<MAXIMO_ATLETAS && tem_med[a[i].id]) {
            char* noc=a[i].noc; int f=-1;
            for(int k=0; k<n; k++) if(strcmp(l[k].chave,noc)==0) f=k;
            if(f!=-1) l[f].valor++; else { if(n<300) { strcpy(l[n].chave,noc); l[n].valor=1; n++; } }
        }
    }
    qsort(l, n, sizeof(ParChaveValor), comparar_decrescente);
    double v[10]; char* lb[10];
    for(int i=0; i<10 && i<n; i++) { printf("#%d %s: %d\n", i+1, l[i].chave, l[i].valor); v[i]=l[i].valor; lb[i]=l[i].chave; }
    gerar_grafico_barra("q22_rank_unicos.svg", lb, v, 10, "Med Unicos", "Qtd"); free(tem_med);
}
